...
class WOFComputerPlayer(WOFPlayer):
    ...
    def smartCoinFlip():
        ...
        if randominteger > self.difficulty:
            return True
        else:
            return False
    ...
    def getPossibleLetters(self,guessed):       
        ...
        for i in LETTERS:
            if i not in guessed and self.prizeMoney > VOWEL_COST:
                Let.append(i)
            elif i not in guessed and self.prizeMoney < VOWEL_COST:
                if i in VOWELS:
                    pass
                else:
                    Let.append(i)
    ...
    def getMove(self,category,obscuredPhrase,guessed):
        ...
        poslet= self.getPossibleLetters(guessed)
        ...
        if self.smartCoinFlip() == True :
            REVERSED_SORTED_FREQUENCIES = SORTED_FREQUENCIES[::-1]
            for char in REVERSED_SORTED_FREQUENCIES:
                if char in Pos_letters:
                    return char
                
        else :
            return random.choice(poslet)
